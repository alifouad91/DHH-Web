...
if ($task === 'overview') { ?>
    ...
    <?php echo $cdh->getDashboardPaneHeaderWrapper('Amenities', false, false, false); ?>
    ...
    <div class="ccm-pane-body">
        <div class="ccm-list-wrapper">
            <?php echo $interface->button('Add Amenity', View::url($configURL . '/add'), 'right', 'primary',
            ...
            <table border="0" cellspacing="0" cellpadding="0" class="ccm-results-list">
                ...
                <?php foreach ($amenities as $i => $amenity) { ?>
                    ...
                    $amenityEditURL = View::url($configURL . '/detail/' . $amenityID);
                    $title          = $amenity->getName();
                    ?>
                    <tr class="ccm-list-record <?php if ($i % 2 === 1) echo 'ccm-list-record-alt'; ?>">
                        <td><a href="<?php echo $amenityEditURL; ?>"><?php echo $th->wordSafeShortText($title, 60); ?></a>
                        </td>
                        <td><a href="<?php echo $this->action('delete', $amenity->getID()); ?>"> Delete </a></td>
                    </tr>
                <?php } ?>
            </table>

        </div>
        <?php $amenityList->displaySummary(); ?>
    </div>

    <div class="ccm-pane-footer">
        <?php $amenityList->displayPagingV2(); ?>
    </div>
...
<?php } else if ($task === 'detail') { ?>
    ...
    ?>
    <?php echo $cdh->getDashboardPaneHeaderWrapper('Edit Amenity', false, false, false); ?>
    <form method="POST" enctype="multipart/form-data" action="<?php echo $this->action('edit_save'); ?>"
          id="amenityForm">
        <?php echo $form->hidden('amenityID', $amenity->getID()); ?>
        ...
        <div class="ccm-pane-body" id="ccm-dashboard-discount-body">
            ...
            <div class="row">
                <div class="span3">
                    <p><strong>Amenity ID</strong></p>
                </div>
                <div class="span8">
                    <p><?php echo $amenity->getId(); ?></p>
                </div>
                ...
                <div class="span8">
                    <p><?php echo $form->text('name', $amenity->getName(), ['style' => 'width:100%;', 'required' =>
                            true]);
                        ?></p>
                </div>
                ...
                <?php if ($amenity->getIcon()) { ?>
                    <div class="span3 thumbnail-amenity">
                        <div class="image">
                            <img src="<?php echo $amenity->getImagePath(); ?>">
                            <div class="top-left">
        ...
        <div class="ccm-pane-footer">
            ...
            <span style="float:right;">
                ...
                <a class='btn danger ' href="<?php echo $this->action('delete', $amenity->getID()); ?>"> Delete </a>
            </span>
        </div>
    </form>
    <?php echo $cdh->getDashboardPaneFooterWrapper(false); ?>
    <script>
        ...
        $('.top-left').click(function () {
            ...
            hiddenField.setAttribute("name", "amID");
            hiddenField.setAttribute("value", amID);
...
<?php } else if ($task == 'add') { ?>
    ...
    <?php echo $cdh->getDashboardPaneHeaderWrapper('Add Amenity', false, false, false); ?>
