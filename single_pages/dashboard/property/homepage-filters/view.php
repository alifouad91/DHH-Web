...
if ($task === 'overview') { ?>
    ...
    /* @var HomePageFiltersList $filterList */

    /** @var TextHelper $th */
    $th = Loader::helper('text');
    ?>

    <?php echo $cdh->getDashboardPaneHeaderWrapper('Homepage Filters', false, false, false); ?>
    ...
    <div class="ccm-pane-body">
        <div class="ccm-list-wrapper">
            <?php echo $interface->button('Add Homepage Filter', View::url($configURL . '/add'), 'right', 'primary',
            ...
            <table border="0" cellspacing="0" cellpadding="0" class="ccm-results-list">
                ...
                <?php foreach ($filters as $i => $filter) { ?>
                    ...
                    $filterEditURL = View::url($configURL . '/detail/' . $filterID);
                    $title           = $filter->getName();
                    ?>
                    <tr class="ccm-list-record <?php if ($i % 2 === 1) echo 'ccm-list-record-alt'; ?>">
                        <td><a href="<?php echo $filterEditURL; ?>"><?php echo $th->wordSafeShortText($title, 60); ?></a>
                        </td>
                        <td><a href="<?php echo $this->action('delete', $filter->getID()); ?>"> Delete </a></td>
                    </tr>
                <?php } ?>
            </table>

        </div>
        <?php $filterList->displaySummary(); ?>
    </div>

    <div class="ccm-pane-footer">
        <?php $filterList->displayPagingV2(); ?>
    </div>
...
<?php } else if ($task === 'detail') { ?>
    ...
    ?>
    <?php echo $cdh->getDashboardPaneHeaderWrapper('Edit Homepage Filters', false, false, false); ?>
    <form method="POST" enctype="multipart/form-data" action="<?php echo $this->action('edit_save'); ?>"
          id="filterForm">
        <?php echo $form->hidden('filterID', $filter->getID()); ?>
        ...
        <div class="ccm-pane-body" id="ccm-dashboard-discount-body">
            ...
            <div class="row">
                <div class="span3">
                    <p><strong>HomePageFilters ID</strong></p>
                </div>
                <div class="span8">
                    <p><?php echo $filter->getId(); ?></p>
                </div>
                ...
                <div class="span8">
                    <p><?php echo $form->text('name', $filter->getName(), ['style' => 'width:100%;', 'required' =>
                            true]);
                        ?></p>
                </div>
        ...
        <div class="ccm-pane-footer">
            ...
            <span style="float:right;">
               <?php echo $form->submit('submit', 'Save Changes', ['class' => 'primary']); ?>
                <a class='btn danger ' href="<?php echo $this->action('delete', $filter->getID()); ?>"> Delete </a>
            </span>
        </div>
    </form>
    <?php echo $cdh->getDashboardPaneFooterWrapper(false); ?>
...
<?php } else if ($task == 'add') { ?>
    ...
    <?php echo $cdh->getDashboardPaneHeaderWrapper('Add Homepage Filters', false, false, false); ?>
