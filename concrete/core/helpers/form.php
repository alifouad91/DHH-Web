...
class Concrete5_Helper_Form
{
    ...
    public function checkbox($key, $value, $isChecked = false, $miscFields = array())
    {
        ...
        return '<input type="checkbox" ' . $this->parseMiscFields('ccm-input-checkbox styled-checkbox', $miscFields) . ' name="' . $key . '" id="' . $id . '" value="' . $value . '" ' . $checked . ' />';
    }
     ...
     * @return $html
     */
    public function text($key, $valueOrArray = false, $miscFields = array() , $disabled = false)
    {
        return $this->inputType($key, 'text', $valueOrArray, $miscFields , $disabled);
    }
     ...
     * @return $html
     */
    public function select($key, $optionValues, $valueOrArray = false, $miscFields = array() , $disabled = false)
    {
        $val = $this->getRequestValue($key);
        if (is_array($val)) {
            $valueOrArray = $val[0];
        ...
        if($disabled)
        {
            $disabled_markup = 'disabled="true"';
        }

        $str = '<select '. $disabled_markup .' name="' . $key . '" id="' . $id . '" ' . $this->parseMiscFields('ccm-input-select', $miscFields) . '>';
