...
class Concrete5_Model_UserInfo extends Object
{
    ...
    */
    ...
    public function updateBillingDetails($uID,$billing_firstName, $billing_lastName, $billing_no, $billing_email, $billing_address, $billing_city, $billing_country)
    {
        $db    = Loader::db();
        $query = "UPDATE UserDetails SET billingFirstName = ?,billingLastName = ?,billingEmail = ?,billingPhone = ?,billingAddress = ?, billingCity = ?, billingCountry = ? WHERE uID = ?";
        $ret   = $db->Execute($query, [$billing_firstName, $billing_lastName, $billing_email, $billing_no, $billing_address, $billing_city, $billing_country, $uID]);
        if ($ret) {
            return true;
        }
    ...
    public function updateReferredDetails($referredBy)
    {
        $db           = Loader::db();
        $creditAmount = Config::get('REFERRAL_CREDIT');
        $query = "UPDATE UserDetails SET referredBy = ?,creditAmount = ? WHERE uID = ?";
        $ret   = $db->Execute($query, [$referredBy, $creditAmount, $this->getUserID()]);
        if ($ret) {
            return true;
        }
    ...
    public function updateReferralCredit($creditAmount)
    {
        $db           = Loader::db();
        $query = "UPDATE UserDetails SET creditAmount = ? WHERE uID = ?";
        $ret   = $db->Execute($query, [$creditAmount, $this->getUserID()]);
        if ($ret) {
            return true;
        }
     ...
     * @return UserInfo
     */
    public static function getByUniqueToken($token)
    {
        $db = Loader::db();
        if ($token) {
            $uID = $db->GetOne('SELECT uID FROM UserDetails WHERE uniqueToken = ?', array($token));
        }
        if ($uID) {
            $ui = UserInfo::getByID($uID);
    ...
    public function updateUniqueToken()
    {
        $db           = Loader::db();
        ...
        $query = "UPDATE UserDetails SET uniqueToken = ? WHERE uID = ?";
        $ret   = $db->Execute($query, [$uniqueToken, $this->getUserID()]);
        if ($ret) {
            return true;
        }
