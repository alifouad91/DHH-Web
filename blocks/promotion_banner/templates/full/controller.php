...
class PromotionBannerBlockController extends BlockController {
	
	protected $btName = 'Promotion Banner';
	protected $btDescription = '';
	protected $btTable = 'btDCPromotionBanner';
	...
	public function view() {
		...
		$this->set('field_8_image', (empty($this->field_8_image_fID) ? null : $this->get_image_object($this->field_8_image_fID, 1400, 370, false)));
	}
...
//WYSIWYG HELPER FUNCTIONS (COPIED FROM "CONTENT" BLOCK):
	function translateFromEditMode($text) {
		...
		$text = preg_replace_callback(
			'/{CCM:FID_([0-9]+)}/i',
			array('PromotionBannerBlockController', 'replaceFileIDInEditMode'),
			$text);


		$text = preg_replace_callback(
			'/{CCM:FID_DL_([0-9]+)}/i',
			array('PromotionBannerBlockController', 'replaceDownloadFileIDInEditMode'),
			$text);
	...
	function translateFrom($text) {
		...
		$text = preg_replace_callback(
			'/{CCM:CID_([0-9]+)}/i',
			array('PromotionBannerBlockController', 'replaceCollectionID'),
			$text);

		$text = preg_replace_callback(
			'/<img [^>]*src\s*=\s*"{CCM:FID_([0-9]+)}"[^>]*>/i',
			array('PromotionBannerBlockController', 'replaceImageID'),
			$text);

		// now we add in support for the files that we view inline
		$text = preg_replace_callback(
			'/{CCM:FID_([0-9]+)}/i',
			array('PromotionBannerBlockController', 'replaceFileID'),
			$text);
		...
		$text = preg_replace_callback(
			'/{CCM:FID_DL_([0-9]+)}/i',
			array('PromotionBannerBlockController', 'replaceDownloadFileID'),
			$text);
