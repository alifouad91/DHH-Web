...
class FormidableValidator extends Formidable {
	...
	public function required($value)
	{
		if (!$this->str->notempty($value))
			$this->add(t('Please enter a valid "%s"'));	
	}
			...
			switch ($type) {
				case 'words':
					...
					if (sizeof($words) < $min || (sizeof($words) > $max && $max > 0))
						if ($max > 0)
							$this->add(t('"%s" should be between %s and %s words', '%s', $min, $max));
						else
							$this->add(t('"%s" should have at least %s words', '%s', $min));
				break;
				
				case 'chars':
					if (!$this->str->min($value, $min) || (!$this->str->max($value, $max) && $max > 0))
						if ($max > 0)
							$this->add(t('"%s" should be between %s and %s charachters', '%s', $min, $max));
						else
							$this->add(t('"%s" should be at least %s charachters', '%s', $min));	
				break;
				
				case 'value':
					if ($value < $min || ($value > $max && $max > 0))
						if ($max > 0)
							$this->add(t('"%s" should be a numeric value between %s and %s', '%s', $min, $max));	
						else
							$this->add(t('"%s" should be a numeric value equal or greater than %s', '%s', $min));
				break;
				
				case 'options':
					if (sizeof($value) < $min || (sizeof($value) > $max && $max > 0))
						if ($min == $max && $max > 0)
							$this->add(t('"%s" should have %s options selected', '%s', $min));
						else
							$this->add(t('"%s" should have between %s and %s options selected', '%s', $min, $max));
				break;
				
				case 'files':
					if (sizeof($value) < $min || (sizeof($value) > $max && $max > 0))
						if ($min == $max && $max > 0)
							$this->add(t('"%s" should have %s files', '%s', $min));
						else
							$this->add(t('"%s" should have between %s and %s files', '%s', $min, $max));
				break;
			}
		}
	}
...
class FormidableValidatorProperty extends FormidableValidator {
	...
	{
		if (!$this->str->notempty($value))
			$this->add(t('Please enter a valid "%s"', t('Label / Name')));	
	}
	...
	public function placeholder($enable, $value)
	{
		if ($enable)
			if (!$this->str->notempty($value))
				$this->add(t('Please enter a valid "%s"', t('Placeholder')));	
	}
	...
	public function default_value($enable, $value)
	{
		if ($enable) 
			if (!$this->str->notempty($value))
				$this->add(t('Please enter a valid "%s"', t('Default')));	
	}
	...
	public function mask($enable, $value)
	{
		if ($enable)
			if (!$this->str->notempty($value))
				$this->add(t('Please enter a valid "%s"', t('Masking')));	
	}
	...
	public function min_max($enable, $min, $max, $type)
	{
		if ($enable) 
		{
			if (!$this->nbr->integer($min))
				$this->add(t('Please enter a valid "%s"', t('Minimum value')));
			
			if (!$this->nbr->integer($max) && $max != '')
				$this->add(t('Please enter a valid "%s"', t('Maximum value')));
			
			if (!$this->str->notempty($type))
				$this->add(t('Please enter a valid "%s"', t('Minimum/Maximum type')));						
		}
	}
	...
	public function tooltip($enable, $value)
	{
		if ($enable)
			if (!$this->str->notempty($value))
				$this->add(t('Please enter a valid "%s"', t('Tooltip')));	
	}
	
	public function tinymce($value)
	{
		if (!$this->str->notempty($value))
			$this->add(t('Please enter a valid "%s"', t('Content')));	
	}
	...
	public function options($values)
	{
		...
		if (!$_hasname)
			$this->add(t('Please enter a valid "%s"', t('Option')));
	}
	...
	public function other($enable, $value, $type)
	{
		...
		{
			if (!$this->str->notempty($value))
				$this->add(t('Please enter a valid "%s"', t('Other option (value)')));
			
			if (!$this->str->notempty($type))
				$this->add(t('Please enter a valid "%s"', t('Other option (type)')));	
		}
	}
	...
	{
		if (!$this->str->notempty($value))
			$this->add(t('Please enter a valid "%s"', t('Code')));	
	}
	
	public function appearance($value)
	{
		if (!$this->str->notempty($value))
			$this->add(t('Please enter a valid "%s"', t('Appearance')));	
	}
	
	public function format($value, $other)
	{
		if (!$this->str->notempty($value))
			$this->add(t('Please enter a valid "%s"', t('Format')));
		else
		{
			if ($value == 'other')
				if (!$this->str->notempty($other))
					$this->add(t('Please enter a valid "%s"', t('Format (other)')));	
		}
	}
	...
	public function advanced($enable, $value)
	{
		if ($enable)
			if (!$this->str->notempty($value))
				$this->add(t('Please enter a valid "%s"', t('Advanced options')));	
	}
	...
	public function allowed_extensions($enable, $value)
	{
		if ($enable) {
			if (!$this->str->notempty($value) || !$this->str->min($value, 2))
				$this->add(t('Please enter a valid "%s"', t('Allowed extensions')));	
			else
			{
	...
	public function fileset($enable, $value)
	{
		if ($enable)
			if (!$this->str->notempty($value))
				$this->add(t('Please enter a valid "%s" is invalid', t('Assign to fileset')));	
	}
	...
	public function css($enable, $value)
	{
		if ($enable)
			if (!$this->str->notempty($value))
				$this->add(t('Please enter a valid "%s"', t('CSS Class')));	
	}
	...
	public function from($from, $reply)
	{
		if ($from['type'] != 'other') 
		{
			if (!$this->str->notempty($from['type']))
				$this->add(t('Please enter a valid %s', t('From')));
		}	
		else 
		{
			if (!$this->str->notempty($from['name']))
				$this->add(t('Please enter a valid %s', t('From (Name)')));
			
			if (!$this->str->email($from['email']))
				$this->add(t('Please enter a valid %s', t('From (Email Address)')));
			
			if ($reply['type'] != 'other') 
			{
				if (!$this->str->notempty($reply['type']))
					$this->add(t('Please enter a valid %s', t('Reply To')));
			}	
			else 
			{
				if (!$this->str->notempty($reply['name']))
					$this->add(t('Please enter a valid %s', t('Reply To (Name)')));
				
				if (!$this->str->email($reply['email']))
					$this->add(t('Please enter a valid %s', t('Reply To (Email Address)')));
			}	
		}	
	}	
	...
	{      
		...
		else
		{
			...
			if (sizeof($emails) <= 0) 
				$this->add(t('Please enter a valid %s', t('Send to (custom)')));   
			else
			{
	...
	{
		if (!$this->str->notempty($value))
			$this->add(t('Please enter a valid "%s"', t('Subject')));	
	}

	public function message($value) 
	{
		if (!$this->str->notempty($value))
			$this->add(t('Please enter a valid "%s"', t('Message')));	
	}
